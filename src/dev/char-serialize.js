...
export const AIM_KEYS = ['style', 'speed', 'color', 'width', 'beams', 'dashes'];
...
export const AIM_WIDTH_RANGE = [0.006, 0.06];
...
function inlineAim(obj) {
  ...
  if (obj.width !== undefined) parts.push(`width: ${num(obj.width)}`);
...
export function pruneCharMeta(table, defaults = {}) {
  ...
  const aimDef = defaults.aim || { style: 'spin', speed: 1, color: '#fff6ec', width: 0.018, beams: 1, dashes: 2 };
  const out = {};
      ...
      if (!e.aim || typeof e.aim !== 'object') problems.push(`${id}.aim: must be object`);
        ...
        for (const [k, v] of Object.entries(e.aim)) {
          if (!AIM_KEYS.includes(k)) problems.push(`${id}.aim: unknown key ${k}`);
            ...
            if (!Number.isFinite(v)) problems.push(`${id}.aim.width: not finite`);
            else {
              const [lo, hi] = AIM_WIDTH_RANGE;
              if (v < lo || v > hi) problems.push(`${id}.aim.width: ${v} out of [${lo},${hi}]`);
            }
