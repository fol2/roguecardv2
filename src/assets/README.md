...
The generated title logotype is `title/title.png`. Live intentionally has no
`title-background/background.png`, so the promoted title screen does not carry
the previous live background. The title-text source, alpha pass, and previews
remain in `scratch/style-tests/title-text-20260706/`.

Gallery cleanup:

- Runtime gallery registration now exposes `live` and `legacy`.
