...
npm test             # test_engine.js (unit checks + 300-run monte-carlo bot) + test_drain_playback.mjs (ms-fast UI drain spy tests)
...
- **Tests are plain self-check scripts**, not a framework. `npm test` runs the whole of `test/test_engine.js` then `test/test_drain_playback.mjs` (plain `node`, `node:assert`). There is no test runner or `-t` filter — to run a subset, comment out blocks in the file. Any assertion failure exits non-zero.
- **`test/test_drain_playback.mjs`** covers `src/ui/drain.js` handlers in Node with spy stubs (no jsdom — `createDrain` is fully dependency-injected). Editing `src/ui/drain.js` or `src/ui/presentation-owners.js` makes `test:fast`/`test:watch` run it first (~40ms) before the heavier Playwright specs. New drain event handlers should add a case here. Deliberate boundary: the heavy choreography handlers (`play`/`hitEnemy`/`die`/`exhaust`/`discardHand`/`enemyAct`) are NOT covered — they need real-DOM fake-element scaffolding and their bug class is visual/timing, already owned by `battle.spec`/`presentation.spec`; add a spy case for one only when an actual wiring bug bites there.
...
- **CI:** `.github/workflows/ci.yml` runs the smoke gate (unit, build dist, browser smoke) for Draft and Ready PRs; label `ci-full` opts a PR into the complete matrix. Pushes to `main` run the complete gate and a red main auto-opens a revert PR (`auto-revert.yml`). The stable aggregate check names are `unit` and `e2e`. `perf.yml` records nightly/manual performance reference metrics without gating the target budget but fails if no valid measurement is produced; `update-baselines.yml` (manual dispatch) regenerates only linux visual baselines as an artifact.
