...
The final JSON reporter annotation for the passing portrait test was:

```text
PERF=1 npx playwright test perf --workers=1 --reporter=json

avg 124.0fps, p95 frame 14.3ms, worst 34.4ms over 372 frames
expected: 2
skipped: 2
unexpected: 0
```
...
The task boundary gate passed without weakening the default Playwright matrix.
`npx playwright test --reporter=list` now runs the mutative BF editor disk-write
test first in the dedicated `bfeditor-disk` dependency project, then runs the
normal desktop/portrait/landscape projects.
...
Playwright: 66 passed, 40 skipped
```
