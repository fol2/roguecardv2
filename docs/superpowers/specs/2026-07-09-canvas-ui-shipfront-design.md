...
| Reward-leave baseline | PR #21 keeps pending rewards durable until the player either claims every offered row or confirms **Leave Rewards Behind?**; a fully claimed reward leaves without the dialog. The engine's pure `pendingRewardHasUntaken()` helper and the reward e2e journeys are protected behaviour. PR #16 cue continuity must span both Continue and confirmation without a duplicate music request |
...
| PR #21's reward-icon wiring leaves stale platform captures after the modular port | Treat reward rendering as loaded predecessor behaviour, inspect the three Darwin reward actual/diff images against the approved PR #21 source and Linux captures, update exactly desktop/portrait/landscape in a separate baseline-only commit after reviewer approval, rerun `48/48`, and keep Task 9's source commit at exactly twenty-two paths |
