...
normalised to `800 x 500`, with a foreground subject/action plus themed
background. Badge, token, and isolated emblem compositions are rejected unless a
card specifically needs a junk-token read.
...
5. Normalise to `800 x 500`.
6. Review at large size and thumbnail size.
7. Put the final image in `src/assets-readable-baseline/cards/<id>.png`.
8. Record the prompt, source output path, final path, and review notes.
9. Run `npm run build`.
