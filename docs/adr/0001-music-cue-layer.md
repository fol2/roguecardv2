...
Spirebound is adding a real BGM layer over the old act ambient drone. We decided on a flat Music Cue registry (`music.play(cueId)`) in `src/music.js`, short crossfades, lazy load + `warmWith`, and independent Music/SFX volume+mute buses with an Audio Panel. Cues may be registered before their content is live (`wired: false`); once content ships, callers flip them live. The drone is retired rather than mixed under BGM.
