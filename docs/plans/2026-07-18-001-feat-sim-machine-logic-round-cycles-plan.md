...
- **R1. Round:** one existing canonical **Climb** from Embark through playable Acts I–III, ending at Dawn or Fall, the two Run Outcomes reachable by the walker. A simulator error is a separate failed-Round status, not a Run Outcome. `Round` is a report alias; it does not replace the `Climb` domain term in game code or player copy.
- **R2. Full Cycle:** an ordered sequence of Rounds sharing one isolated Vigil, starting from a fresh Vigil and ending when the canonical `act4Reveal` sealed-door promise is durably staged to a specific Round/run id, when `maxRounds` is reached and the cycle is reported as censored, or when a simulator error terminates the cycle as failed. A failed cycle retains the errored Round and reproduction evidence, starts no later Round from possibly partial state, and is excluded from completion-timing statistics. Six Shards / `isRevealed(vigil, 'act4')` is the unlock threshold; durable staging, not visual playback, is the completion endpoint.
...
- **Test scenarios:** AE1–AE5 and AE7 synthetic aggregates; direct reduction equals split/merge; 1/N partition reports match; completed-only percentiles exclude censored/failed cycles; Kaplan-Meier and restricted mean include censor cells; suffix denominators include only attempted Rounds and cluster-bootstrap output is deterministic; invalid funnel ordering or reason vocabulary fails smoke; stable serialisation is byte-identical.
